...
3.  **Enhance Agent Prompting:**
    ...
    *   *Example Prompt Addition:* "You have a set of tools prefixed with `browser.` to interact with a live web browser. Use `browser.get_state` to see the current page and its interactive elements. Use `browser.click(index)` and `browser.type(index, text)` to interact with those elements."
